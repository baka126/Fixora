...
// gatherDependencyContext discovers Git repositories for related resources.
func (c *Controller) gatherDependencyContext(ctx context.Context, pod *v1.Pod) map[string]*argocd.AppInfo {
	deps := make(map[string]*argocd.AppInfo)
...
func (c *Controller) handleRemediation(ctx context.Context, pod *v1.Pod, evidence notifications.EvidenceChain, diagnosis Diagnosis, investigationID int64) {
	...
	if aggregatedContext.Len() == 0 {
		slog.Warn("No files gathered from dependencies for remediation")
		return
	}
	...
	for repoKey, changes := range patchesByRepo {
		if repoInfo, ok := repoMap[repoKey]; ok {
			...
			if c.config.PolicyGuardrailsEnabled {
				if err := enforcePatchGuardrails(repoInfo.Source, changes, c.config.AllowedImageRegistries, c.config.ExcludedNamespaces); err != nil {
