...
const (
	...
	PromptGeneratePatch       = "You are a Kubernetes GitOps expert. Generate ONLY the complete new file content. No markdown.\n\n[CURRENT CONTENT]\n%s\n\n[EVIDENCE]\n%s\n\nYou MUST respond with a JSON object containing: 'patch' (the full file content) and 'confidence' (a percentage from 0 to 100 representing your certainty that this patch is correct and safe)."
...
type Provider interface {
	...
	GeneratePatch(ctx context.Context, currentContent []byte, evidence string) (AIResponse, error)
}
